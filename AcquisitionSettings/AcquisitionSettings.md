...
3. `Stop on *.elf change` - if checked the acquisition will be stopped when the *.elf file is changed (whenever you recompile)
4. `Sampling` - set the sampling frequency in Hz. Depending on your debug probe setup this can be reached or not. 
5. `Max points` - number of points in the circular buffer collecting the data. When acquisition is longer the oldest points are going to be lost. At low sampling frequencies you can use {ref}`Logging` to stream the data to a file.
6. `Viewport width` - width of the plot viewport in milliseconds during acquisition.
...
2. `Connection` (JLink only) - select connection type USB or TCP/IP
3. `Debug probe serial number` - select a serial number of your debug probe
4. `SWD speed in kHz` - select a SWD speed in kHz. Be careful with too high values as they can cause target resets. Too low values will lower the maximum data rate. The recorder module is not affected by this setting.
5. `Target name` (JLink only) - select your target name. For some targets there might be a CPU halt issued on acquisition start. In such cases select a generic device, for example Cortex-M4 instead of STM32G474.
6. `Mode` (JLink only) - select either normal or [HSS](https://kb.segger.com/J-Link_High-Speed_Sampling) mode. Normal mode will work on all JLink probes, however it's much slower. HSS mode allows to reach very high update frequencies (tens of kilohertz), however it's limited to certain JLink types. The recorder module is not affected by this setting.
7. `JLink script` (JLink only) - select checkbox and set path to a custom [JLink-script](https://kb.segger.com/J-Link_script_files) if needed. 
8. `Keep connection` - if checked the connection will be kept open after acquisition is stopped. With some probes it's necessary to avoid target halts when restarting. Uncheck if CPU halts are not problematic in your application - this makes programming easier as you don't have to manually disconnect using the main screen top right status button. 
...
```{warning}
Logging can decrease the performance when using high frequency sampling. Logging cannot be used with the recorder module.
```

## Advanced section


Since version 1.2.7 an alternative *.elf parser is available that is much faster than GDB. It can be turned on by selecting the checkbox (1).

Otherwise GDB is used to parse *.elf file. In case you'd like to select a different gdb instance on your PC you can change it in the `GDB command` field (2).

```{figure} ./images/AcquisitionAdvanced.png
:width: 1000px
:align: center
```