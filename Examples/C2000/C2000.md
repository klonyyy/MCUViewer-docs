...
Adding the serial driver is pretty straightforward - simply copy the `serialDriver` folder to the project and adjust the `serialDriverDefines.h` file to match your system configuration. The `serialDriver` folder can be downloaded together with MCUViewer executable [here](https://mcuviewer.com/#downloads). 
